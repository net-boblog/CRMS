...
<form id="aroleForm"> <%--action="/user/insert.htm" method="post" onsubmit="return checkForm()">--%>
  <table class="table table-bordered table-striped table-condensed">
    ...
    <tr>
      <th scope="row">角色名：</th>
      <td><input id="roleName" class="form-control" type="text" name="roleName" value="${role.roleName}" placeholder="请输入角色名" required>
        <div>
    ...
    <tr>
      <th scope="row">说明：</th>
      <td><input id="roleDes" class="form-control" type="text" name="roleDescript" value="${role.roleDescript}" placeholder="请输入角色说明" required>
        <div>
          <span id="userError"></span>
          <%--<span font color="red">${errorMsg2}</span>--%>
        </div>
...
<script type="text/javascript">
  $(function() {
    $("#auserBtn").click(function () {
      ...
      if ($("#roleDes").val() == "") {
        ...
        $.ajax({
          type: "POST",
          url: "/role/updateMenu.htm?roleId=${roleId}",
  ...
  var setting = {
    ...
    async: {
      enable: true,
      type: 'post',
      url: "/role/updateRole.htm?roleId=${roleId}"
    },
    data: {
